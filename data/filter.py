...
def parse_arguments():
	...
	arg_parser.add_argument('-id', '--include_domains', help='comma-separated list of domains to include (default: All)')
	arg_parser.add_argument('-ed', '--exclude_domains', help='comma-separated list of domains to exclude (default: None)')
	arg_parser.add_argument('-xd', '--exact_domains', help='comma-separated list of exact domain set to include (default: All)')
	# random selection
...
def main():
	...
	# load predicted domain distribution
	domain_distribution = None
	if args.domain_distribution:
		with open(args.domain_distribution, 'rb') as fp:
			domain_distribution = pickle.load(fp)

	# setup domain filters
	if args.include_domains:
		include_domains = [d.strip() for d in args.include_domains.split(',')]
		filters.append(UniversalDependenciesDomainFilter(include_domains, source=domain_distribution, mode='include'))
	if args.exclude_domains:
		exclude_domains = [d.strip() for d in args.exclude_domains.split(',')]
		filters.append(UniversalDependenciesDomainFilter(exclude_domains, source=domain_distribution, mode='exclude'))
	if args.exact_domains:
		exact_domains = [d.strip() for d in args.exact_domains.split(',')]
		filters.append(UniversalDependenciesDomainFilter(exact_domains, source=domain_distribution, mode='exact'))
